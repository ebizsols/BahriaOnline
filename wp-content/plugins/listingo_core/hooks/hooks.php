 ...
 * @return 
 */
if (!function_exists('listingo_get_domain')) {
	add_filter('listingo_get_domain','listingo_get_domain',10,1);
	function listingo_get_domain(){
    ...
    function listingo_user_registration($atts = '') {
        ...
        if (isset($_POST['register']['account']) && esc_attr($_POST['register']['account']) === 'seeker') {
			...
			if (!wp_verify_nonce($_POST['security'], 'ajax_nonce')) {
				$json['type'] = 'error';
				$json['message'] = esc_html__('Security check failed, this could be because of your browser cache. Please clear the cache and check it again', 'listingo_core');
				wp_send_json( $json );
			}
		...
		if (isset($_POST['register']['type']) && esc_attr($_POST['register']['type']) === 'business') {
			$user_nicename	= !empty( $company_name ) ? $company_name : $user_nicename;
		} else{
			$user_nicename	= !empty( $full_name ) ? $full_name : $user_nicename;
		}
