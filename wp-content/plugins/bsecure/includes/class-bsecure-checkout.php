...
const BSECURE_LIVE_VIEW_ORDER_URL = 'https://partner.bsecure.pk/view-order/';
	...
	public function __construct(){
		...
		add_action( 'wp_ajax_nopriv_ajax_order_to_bsecure', array($this, 'ajax_order_to_bsecure' ) ); // for none admin or guests
    ...
    //Add a custom field (in an order) to the admin order detail page
		...
		add_action( 'plugins_loaded', array($this, 'load_bsecure_gateway_class' ) );		
			...
			if(!empty($order_data->order_ref)){
				...
				if(!empty($wc_order)){			
					...
					if ($placement_status == Bsecure_Checkout::BSECURE_CREATED_STATUS 
						...
						$return_json = ['status' => true, 'msg' =>__("Sorry! your order has not been proccessed.","wc-bsecure"), 'bsecure_order_id' => $bsecure_order_id];
						...
						if($order_id > 0){
							...
							if($order->get_status() == Bsecure_Checkout::STATUS_CANCELED || $order->get_status() == Bsecure_Checkout::STATUS_FAILED){
								...
								$return_json = ['status' => true, 'msg' => __("Order added successfully at woocommerce.", "wc-bsecure"), 'bsecure_order_id' => $bsecure_order_id];

							}							

						}else{

							$return_json = ['status' => false, 'msg' => __("Unable to create order at woocommerce. Please contact administrator or retry","wc-bsecure")];
						}
    ...
    $long         	= "";
	  ...
	  // Get the customer country code
	    ...
	    if (!empty($order_data->summary->discount_amount)) {
	    	...
	    	$discount = ($order_data->summary->discount_amount);
	    	...
	    	$discount = $discount >= ($total) ? -$total : -$discount;	    	
				...
				$item_fee->set_name( __("bSecure Discount") ); // Generic fee name
	    ...
	    if (!empty($order_data->summary->merchant_service_charges)) {
		    ...
		    $merchant_service_charges = abs($order_data->summary->merchant_service_charges);

		    // Get a new instance of the WC_Order_Item_Fee Object
				$item_fee = new WC_Order_Item_Fee();
				$item_fee->set_name( __("Service Charges") ); // Generic fee name
				$item_fee->set_amount( $merchant_service_charges ); // Fee amount
				$item_fee->set_total( $merchant_service_charges ); // Fee amount				
		...
		if( $validateResponse['error'] ){				
			...
			if( $validateResponse['error'] ){				
				...
				if(!empty($response->body->order_reference)){				
					...
					echo json_encode(['status' => true, 'msg' => __("Request Success", 'wc-bsecure'), 'redirect' => $redirect]);
	...
	*/
	...
	public function updateOrderPaymentGateway($order, $order_data, $customer){
	  ...
	  // Get the customer country code
	    ...
	    $merchant_service_charges = abs($order_data->summary->merchant_service_charges);
			$item_fee = new WC_Order_Item_Fee();
			$item_fee->set_name( __("Service Charges") ); 
			$item_fee->set_amount( $merchant_service_charges ); 			
			$item_fee->set_total( $merchant_service_charges );	
			$order->add_item( $item_fee );
			$order->calculate_totals();
    ...
    $countries = $countries_obj->get_allowed_countries();
	    ...
	    if(get_option('wc_auto_append_country_code', 'no') == 'yes' ){
		   ...
		   $fields['billing']['country_calling_code']['default'] = $default_country_code;
	...
	public function bsecure_adding_country_prefix_on_billing_phone(){
	    ...
	    <script type="text/javascript">
	        ...
	        ( function( $ ) {
	        	...
	        	var default_country_code = "<?php echo $default_country_code; ?>";
	            ...
	            $( document.body ).on( 'updated_checkout', function(data) {
		            ...
		            if(payment_method === 'bsecures' || auto_append_country_code == 'yes'){	       
		                ...
		                $.post( bsecure_js_object.ajax_url, ajax_data, function( response ) {                	

		                	if(default_country_code !== ''){

		                		$('#country_calling_code').val(default_country_code);
		                		default_country_code = '';

		                	}else{

		                		$('#country_calling_code').val(response.calling_code);
