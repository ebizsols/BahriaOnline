            ...
            if ($user) {
				...
				if( !empty( $payment_type ) && $payment_type === 'booking' ){
					...
					if (class_exists('ListingoProcessNotification')) {
						...
						$emailData['appointment_date'] = $earnings_data['appointment_date'];
					...
					if (class_exists('ListingoProcessNotification')) {
						...
						$emailData['address'] 		= do_shortcode($billing_address);
...
if (!function_exists('listingo_get_package_features')) {
    ...
    function listingo_get_package_features($key='') {
        $features = array(
			...
			'sp_banner' 		=> esc_html__( 'Profile banner includes?', 'listingo' ), 
			...
			foreach( $items as $key => $order_item ){
				...
				if( !empty( $order_detail ) ) {
					...
					<div class="row">
						<div class="col-md-12">
							<div class="cart-data-wrap">
							  <h3><?php echo esc_html($name);?>( <span class="cus-quantity">×<?php echo esc_attr( $quantity );?></span> )</h3>
							  <div class="selection-wrap">
								<?php 
									$counter	= 0;
				...
				<table class="cus-table" style="background:#fbfbfb; margin:auto 0; width:600px; border-spacing:0; border-radius: 3px;">
					<tbody>
						<tr style="text-align:left; border:0; line-height: 2.5; font-family: 'Helvetica Neue', 'Helvetica', Helvetica, Arial, sans-serif;">
							<td scope="col" style="text-align:left; padding:0 15px; border:0; border-bottom:1px solid #ececec; line-height: 2.5; font-size: 20px; font-weight: bold;"><?php echo esc_html($name);?>( <span class="cus-quantity">×<?php echo esc_attr( $quantity );?></span> )</td>
						</tr>
						<tr style="text-align:left; border:0; line-height: 2.5; font-family: 'Helvetica Neue', 'Helvetica', Helvetica, Arial, sans-serif;">
								...
								<table style="width:100%; margin:0; border-spacing:0;">
									<tbody>
										...
										<tr style="line-height: 2.5; font-family: 'Helvetica Neue', 'Helvetica', Helvetica, Arial, sans-serif;">
											...
											<td scope="col" style="width:50%; text-align:left; padding:0 15px; border:0; border-bottom:1px solid #ececec; line-height: 2.5;"><?php echo esc_html($value);?></td>
										</tr>
										<?php }?>
									</tbody>
								</table>
