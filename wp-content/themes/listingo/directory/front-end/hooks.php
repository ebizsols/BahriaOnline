                ...
                <div class="tg-feedback" style="display:block;">
                    <figure>
                        <a href="<?php echo esc_attr($author_url); ?>">
                            <img src="<?php echo esc_url($avatar); ?>" alt="<?php esc_attr_e('Reviewer', 'listingo'); ?>">
                        </a>
                    </figure>
                            ...
                            <div class="tg-contenthead">
                                <div class="tg-leftbox">
                                    ...
                                    <ul class="tg-matadata">
                                        <li> <a href="<?php echo esc_attr($author_url); ?>"><?php echo esc_html($reviewer_name); ?></a></li>
                                        <li>
                                            <a href="javascript:;">
                                            ...
                                            <div class="tg-overallrating">
                                                <?php if (!empty($rating_titles)) { ?>
                                                    <ul class="tg-servicesrating">
                                                       <?php echo do_shortcode( $review_time );?>
                                                        <?php
...
if (!function_exists('listingo_get_profile_views')) {
    ...
    function listingo_get_profile_views($user_id, $key = '') {
		$count = get_user_meta($user_id, $key, true);
        if (!empty($_COOKIE[$key . $user_id])) {
            //do nothing
        }else{
			setcookie($key . $user_id, 'profile_view_count', time() + 3600);
            if (empty($count)) {
                $count = 1;
                update_user_meta($user_id, $key, $count);
            } else {
...
if (!function_exists('listingo_get_total_rating_votes')) {
    ...
    function listingo_get_total_rating_votes($author_id, $return_type = 'echo') {
		...
		if( !empty($enable_reviews['gadget']) && $enable_reviews['gadget'] === 'enable' ){
			if( isset( $rating_visibility ) && $rating_visibility === 'registered_only' && !is_user_logged_in() ){
				...
				<ul class="tg-matadata">
					...
					<li>
						...
						<em><?php echo esc_html($recommendations); ?> <?php echo esc_attr($total_votes); ?></em>
					</li>
				</ul>
				<?php
				$output_data = ob_get_clean();
...
if (!function_exists('listingo_get_team_members')) {
    ...
    function listingo_get_team_members() {
		...
		$users_found = $users_query->get_total(); //Total Users
...
if (!function_exists('listingo_invite_users')) {
    ...
    function listingo_invite_users() {
        ...
        if (class_exists('ListingoProcessNotification')) {
            ...
            $emailData['message'] = do_shortcode($message);
...
if (!function_exists('listingo_get_search_keyword')) {
    ...
    function listingo_get_search_keyword() {
        ...
        ?>
        <input type="text" name="keyword" value="<?php echo esc_attr($keyword); ?>" class="form-control" placeholder="<?php esc_attr_e('Keyword', 'listingo'); ?>">
        <?php
        echo ob_get_clean();
    }
...
if (!function_exists('listingo_get_search_zip')) {
    ...
    function listingo_get_search_zip() {
        ...
        ?>
        <input type="text" name="zip" value="<?php echo esc_attr($zip); ?>" class="form-control" placeholder="<?php esc_attr_e('Zip code', 'listingo'); ?>">
        <?php
        echo ob_get_clean();
    }
...
if (!function_exists('listingo_get_search_geolocation')) {
    ...
    function listingo_get_search_geolocation() {
        ...
        <div class="locate-me-wrap">
            ...
            <input type="text"  autocomplete="on" id="location-address" value="<?php echo esc_attr($location); ?>" name="geo" placeholder="<?php esc_attr_e('Geo location', 'listingo'); ?>" class="form-control">
...
if (!function_exists('listingo_get_ads_search_geolocation')) {
    ...
    function listingo_get_ads_search_geolocation() {
        ...
        <div class="locate-me-wrap">
            ...
            <input type="text"  autocomplete="on" id="location-address-ads" value="<?php echo esc_attr($location); ?>" name="geo" placeholder="<?php esc_attr_e('Geo location', 'listingo'); ?>" class="form-control">
            <?php if (isset($dir_radius) && $dir_radius === 'enable') { ?>
...
if (!function_exists('listingo_get_header_search_geolocation')) {
    ...
    function listingo_get_header_search_geolocation() {
        ...
        <div class="locate-me-wrap">
            ...
            <input type="text"  autocomplete="on" id="location-address-header" value="<?php echo esc_attr($location); ?>" name="geo" placeholder="<?php esc_attr_e('Geo location', 'listingo'); ?>" class="form-control">
            <?php if (isset($dir_radius) && $dir_radius === 'enable') { ?>
...
if (!function_exists('listingo_get_search_filtrs')) {
    ...
    function listingo_get_search_filtrs() {
		...
		$amenities_list = listingo_get_taxonomy_array('amenities');
		...
		if (!empty($_GET['amenities'])) {
		   $get_amenities = !empty($_GET['amenities']) ? $_GET['amenities'] : '';
		} else {
			if ( is_tax('amenities') ) {
				$sub_cat = $wp_query->get_queried_object();
				if( !empty( $sub_cat->slug ) ){
					$get_amenities = array( $sub_cat->slug );
				}
			} else{
				$get_amenities = array();
			}
					...
					<div class="tg-checkboxgroup data-list">
						...
						foreach ($languages_list as $pterm) {
							...
							<span class="tg-checkbox sp-load-item">
								...
								<label for="lang-<?php echo esc_attr($pterm->slug); ?>"><?php echo esc_html($pterm->name); ?></label>
							</span>
							<?php
						}
						?>
		...
		<?php if (!empty($amenities_list)) { ?>
			<fieldset class="amenity-search-wrap">
				<h4><?php esc_html_e('Filter By Amenities', 'listingo'); ?></h4>
				<div class="tg-checkboxgroup data-list">
					<?php
					foreach ($amenities_list as $pterm) {
						$checked = '';
						if (!empty($get_amenities) && in_array($pterm->slug, $get_amenities)) {
							$checked = 'checked';
						}
						?>
						<span class="tg-checkbox sp-load-item">
							<input type="checkbox" <?php echo esc_attr($checked); ?> id="amt-<?php echo esc_attr($pterm->slug); ?>" name="amenities[]" value="<?php echo esc_attr($pterm->slug); ?>">
							<label for="amt-<?php echo esc_attr($pterm->slug); ?>"><?php echo esc_html($pterm->name); ?></label>
						</span>
						<?php
					}
					?>
					<?php if (listingo_count_items($amenities_list) > 10) { ?>
						<a href="javascript:;" class="sp-loadMore"><?php esc_html_e('Load More', 'listingo'); ?></a>
					<?php } ?>
				</div>
				<?php
				wp_add_inline_script('listingo_callbacks', "_show_hide_list('.amenity-search-wrap');", 'after');
				?>
			</fieldset>
		<?php } ?>
					...
					<div class="tg-checkboxgroup data-list">
						...
						foreach ($insurance_list as $pterm) {
							...
							<span class="tg-checkbox sp-load-item">
								...
								<label for="insurance-<?php echo esc_attr($pterm->slug); ?>"><?php echo esc_html($pterm->name); ?></label>
							</span>
							<?php
						}
						?>
									...
									<div class="tg-filterdetails">
										<div class="tg-themeform tg-filterform">
											<div class="tg-haslayout spv-filters-ad">
													...
													<?php if (!empty($languages_list)) { ?>
														<div class="tg-filterholder tg-filterbg">
															...
															<div class="form-group tg-searchholder">
																<span class="tg-select">
																	<span class="tg-searchholdericon"><i class="lnr lnr-magnifier"></i><?php esc_html_e('Select Language', 'listingo'); ?>:</span>
																	<select data-placeholder="<?php esc_attr_e('None', 'listingo'); ?>" class="chosen-lang-select" multiple name="languages[]">
																		<?php
																			foreach ($languages_list as $pterm) {
																				$checked = '';
																				if (!empty($get_languages) && in_array($pterm->slug, $get_languages)) {
																				...
																				?>
																				<option <?php echo esc_attr($checked); ?> value="<?php echo esc_attr($pterm->slug); ?>"><?php echo esc_html($pterm->name); ?></option>
																		<?php }?>
																	</select>
																</span>
															</div>
												...
												<?php if (!empty($amenities_list)) { ?>
													<div class="tg-filterholder tg-filterbg">
														<div class="tg-title"><h4><?php esc_html_e('Filter By Amenities', 'listingo'); ?> :</h4></div>
														<div class="form-group tg-searchholder">
															<span class="tg-select">
																<span class="tg-searchholdericon"><i class="lnr lnr-magnifier"></i><?php esc_html_e('Select Amenities', 'listingo'); ?>:</span>
																<select data-placeholder="<?php esc_attr_e('None', 'listingo'); ?>" class="chosen-lang-select" multiple name="amenities[]">
																	<?php
																		foreach ($amenities_list as $pterm) {
																			$checked = '';
																			if (!empty($get_amenities) && in_array($pterm->slug, $get_amenities)) {
												...
												<?php if (!empty($dir_search_insurance) && $dir_search_insurance === 'enable') { ?>
													...
													<div class="tg-filterholder">
														...
														<div class="form-group tg-searchholder">
															<span class="tg-select">
																<span class="tg-searchholdericon"><i class="lnr lnr-magnifier"></i><?php esc_html_e('Select Insurance', 'listingo'); ?>:</span>
																<select data-placeholder="<?php esc_attr_e('None', 'listingo'); ?>" name="insurance[]" multiple class="chosen-insurance-select">
																	<?php
																	foreach ($insurance_list as $pterm) {
																		$checked = '';
																		if (!empty($get_insurance) && in_array($pterm->slug, $get_insurance)) {
...
if (!function_exists('listingo_result_avatar')) {
    ...
    function listingo_result_avatar($user_id, $media_type = 'avatar') {
        ...
        <figure class="tg-featuredimg">
            <a href="<?php echo esc_url(get_author_posts_url($user_id)); ?>"><img class="searched-avatar" src="<?php echo esc_url($thumb); ?>" alt="<?php esc_attr_e('Listingo', 'listingo'); ?>"></a>
    ...
    function listingo_result_avatar_v2($user_id, $classes = '',$size=array('width' => 275, 'height' => 152)) {
        ...
        <figure class="tg-featuredadimg <?php echo esc_attr($classes); ?>">
			<a href="<?php echo esc_url(get_author_posts_url($user_id)); ?>">
				...
				<?php } ?>
				<img src="<?php echo esc_url( $banner_thumb );?>" alt="<?php esc_attr_e('banner', 'listingo'); ?>">
				<figcaption>
					<span class="tg-profileimg"><img src="<?php echo esc_url( $avatar_thumb );?>" alt="<?php esc_attr_e('avatar', 'listingo'); ?>"></span>
				</figcaption>
			</a>
		</figure>
        <?php
    ...
    function listingo_result_tags($user_id, $return_type = 'echo') {
        ...
        if ( (!empty($verify_user) && $verify_user === 'on' )
            ...
            <ul class="tg-tags <?php echo esc_attr( $custom_styling );;?>">
                <?php if (isset($featured_expiry) && $featured_expiry > 0) { ?>
                    <li><span <?php echo do_shortcode($feature_style );?> class="tg-tag tg-featuredtag"><i class="fa fa-star sp-tags-icon"></i><?php esc_html_e('Featured', 'listingo'); ?></span></li>
                <?php } ?>
                <?php if (!empty($verify_user) && $verify_user === 'on') { ?>
                    <li><span <?php echo do_shortcode($verified_style );?> class="tg-tag tg-verifiedtag"><i class="fa fa-check sp-tags-icon"></i><?php esc_html_e('Verified', 'listingo'); ?></span></li>
                <?php } ?>
            </ul>
            <?php
        }
...
if (!function_exists('listingo_make_an_appointment_button')) {
    ...
    function listingo_make_an_appointment_button($user_id) {
        ...
        ?>
       	 <a class="tg-btn" href="<?php echo esc_url(get_author_posts_url($user_id)); ?>"><?php echo esc_html( $title ); ?></a>
        <?php
        echo ob_get_clean();
    }
					...
					if (!empty($userphone)) {
						...
						<li>
							...
							<?php if( !empty($infoformat) && $infoformat === 'full' ){?>
								<span><a href="tel:<?php echo esc_attr($userphone); ?>"><?php echo esc_html($userphone); ?></a></span>
							<?php }else{?>
								<span data-toggle="tooltip" data-placement="top" title="<?php esc_attr_e('Click to call', 'listingo'); ?>" data-last="<?php echo esc_attr($phone_2); ?>"><a href="tel:<?php echo esc_attr($userphone); ?>"><em>XXXX</em><?php echo esc_html($phone); ?></a></span>
							<?php }?>
				...
				if ( apply_filters('listingo_is_contact_informations_enabled', 'yes','yes',$user->ID) === 'yes') {
					...
					if( !empty( $email ) ){
						...
						<li>
							<i class="lnr lnr-envelope"></i>
							<?php if( !empty($infoformat) && $infoformat === 'full' ){?>
								<span><a href="mailto:<?php echo esc_attr($useremail); ?>"><?php echo esc_html( $useremail ); ?></a></span>
							<?php }else{?>
								<span data-toggle="tooltip" data-placement="top" title="<?php esc_attr_e('Click to view email', 'listingo'); ?>" data-last="<?php echo esc_attr($email[1]); ?>"><a href="mailto:<?php echo esc_attr($useremail); ?>"><?php echo esc_html($email[0]); ?>@<em>XXXX.XXXX</em></a></span>
							<?php }?>
						</li>
					<?php } }
				}
		...
		if( !empty( $remove_qr ) && $remove_qr === 'no' ){
			...
			<div class="tg-authorcodescan">
				<div class="tg-qrscan">
					<figure>
						<img class="tg-qr-img" 
						src="<?php echo esc_url( get_template_directory_uri()) ; ?>/images/qrcode.png" 
						alt="<?php esc_attr_e('image-discripton', 'listingo'); ?>">
						<figcaption>
				...
				<div class="col-xs-12 col-sm-12 col-md-6 col-lg-3 tg-verticaltop hidden">
					<div class="tg-serviceprovider">
						...
						<div class="tg-companycontent">
							...
							<div class="tg-title">
								<h3><a href="<?php echo esc_url(get_author_posts_url($user->ID)); ?>"><?php echo esc_html($username); ?></a></h3>
							</div>
 ...
 * @return 
 */
if (!function_exists('listingo_disable_fw_use_sessions')) {
	add_filter('fw_use_sessions','listingo_disable_fw_use_sessions');
	function listingo_disable_fw_use_sessions(){
		return false;
	}
}
 ...
 * @return 
 */
if( !function_exists( 'listingo_load_footer_contents' ) ){
	add_action('wp_footer', 'listingo_load_footer_contents');
	function listingo_load_footer_contents(){
		//Forgot password 
		if (!is_user_logged_in()) {
