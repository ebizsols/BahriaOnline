...
if (!function_exists('listingo_process_job')) {
    ...
    function listingo_process_job() {
        ...
        if (function_exists('fw_get_db_settings_option')) {
			...
			$post_job_emails = fw_get_db_settings_option('post_job_emails', $default_value = null);
        }

        $profile_page 		= isset($dir_profile_page[0]) ? $dir_profile_page[0] : '';
		$post_job_emails 	= !empty($post_job_emails) ? $post_job_emails : 'no';
        ...
        if (isset($type) && $type === 'add') {
			...
			$json['return_url']	= wp_specialchars_decode($return_url);
			
			if(!empty($post_job_emails) && $post_job_emails === 'yes'){
                ...
                if (!empty($list_job_types)) {
                    foreach ($list_job_types as $key => $value) {
                        ...
                        <span class="tg-checkbox">
                            ...
                            <label for="tg-<?php echo esc_attr($key); ?>"><?php echo esc_html($value); ?></label>
                        </span>
                        <?php
                    }
                }
