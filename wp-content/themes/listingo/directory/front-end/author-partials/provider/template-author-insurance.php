   	...
   	<div class="spv4-insurance layout-default" id="section-insurance">
		<div class="tg-companyfeaturebox tg-insurance-wrap">
			<div class="tg-companyfeaturetitle">
				<h3><?php echo esc_html( $username );?>&nbsp;<em><?php esc_html_e('get given below insurance plan', 'listingo'); ?></em></h3>
			</div>
			<ul>
				<?php
				foreach ($list_insurance as $key => $insurance) {
					...
					<li class="tg-activated <?php echo esc_attr( $ins_img );?>">
						<?php if( !empty( $insurance_meta['logo']['url'] ) ){?>
							<img src="<?php echo esc_url($insurance_meta['logo']['url']); ?>" alt="<?php esc_attr_e('insurance', 'listingo'); ?>">
						<?php }?>
						<span><?php echo esc_html($insurance_title); ?></span>
					</li>
					<?php
				}
				?>
