...
<div class="sc-cities tg-haslayout tg-paddingzero">
	<div class="row">
			...
			<?php 
				...
				if ( !empty( $terms ) ){ 							
					foreach ( $terms as $key => $value ) {
						...
						$cat_image = !empty( $term_data['image']['url'] ) ? $term_data['image']['url'] : esc_url( get_template_directory_uri()).'/images/locations/city.jpg';
					...
					<div class="col-xs-6 col-sm-6 col-md-3 col-lg-3 tg-verticaltop">
						<div class="tg-topcity">
							<figure class="tg-cityimg">									
								<img src="<?php echo esc_url( $cat_image ); ?>" alt="<?php esc_attr_e('City Image', 'listingo'); ?>">									
								<figcaption>										
									<h3><a href="<?php echo esc_url( get_term_link( $value->term_id, 'cities' ) ); echo esc_attr( $custom_url  ); ?>"><?php echo esc_html( $value->name ); ?></a></h3>
									<span><?php echo esc_html( $total_users ); ?>&nbsp;<?php esc_html_e('Listings', 'listingo'); ?></span>										
								</figcaption>
							</figure>
						</div>
					</div>
		...
		?>	
			<div class="tg-btnbox">
				<a class="tg-btn tg-btnviewall" href="<?php echo esc_attr( $btn_link ); ?>"><?php echo esc_html( $btn_text ); ?></a>
			</div>
		<?php } } } ?>			
	</div>
</div>				