...
if (!function_exists('crework_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'crework_hovers_theme_setup3', 3 );
	function crework_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		crework_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'crework'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'crework') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'crework'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'crework'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'crework') ),
				"std" => 'dots',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Default',	'crework')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('crework_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'crework_hovers_theme_setup9', 9 );
	function crework_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'crework_hovers_frontend_scripts', 1010 );
		add_filter( 'crework_filter_localize_script','crework_hovers_localize_script' );
		add_filter( 'crework_filter_merge_scripts',	'crework_hovers_merge_scripts' );
		add_filter( 'crework_filter_merge_styles',	'crework_hovers_merge_styles' );
		add_filter( 'crework_filter_get_css', 		'crework_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'crework_hovers_frontend_scripts' ) ) {
	
	function crework_hovers_frontend_scripts() {
		if ( crework_is_on(crework_get_theme_option('debug_mode')) && crework_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'crework-hovers', crework_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( crework_is_on(crework_get_theme_option('debug_mode')) && crework_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'crework-hovers',  crework_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('crework_hovers_merge_scripts')) {
	
	function crework_hovers_merge_scripts($list) {
...
if (!function_exists('crework_hovers_merge_styles')) {
	
	function crework_hovers_merge_styles($list) {
...
if (!function_exists('crework_hovers_localize_script')) {
	
	function crework_hovers_localize_script($arr) {
		$arr['button_hover'] = crework_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('crework_hovers_add_icons') ) {
	function crework_hovers_add_icons($hover, $args=array()) {
					...
					echo apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'crework') : '')
										. '</a>',
										$product );
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'crework');
						else
							esc_html_e('Details', 'crework');
					}
				?></a>
			</div>
			<?php
...
// Add styles into CSS
if ( !function_exists( 'crework_hovers_get_css' ) ) {
	
	function crework_hovers_get_css($css, $colors, $fonts, $scheme='') {
