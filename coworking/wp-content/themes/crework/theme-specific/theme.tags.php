...
if ( !function_exists('crework_need_page_title') ) {
	function crework_need_page_title() {
		return !is_front_page() && apply_filters('crework_filter_need_page_title', true);
	}
}
...
if ( !function_exists('crework_show_layout') ) {
	function crework_show_layout($str, $before='', $after='') {
		if (trim($str) != '') {
...
if ( !function_exists('crework_get_logo_image') ) {
	function crework_get_logo_image($type='') {
		$logo_image = '';
		if (crework_get_retina_multiplier(2) > 1)
			$logo_image = crework_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '').'_retina' );
		if (empty($logo_image)) 
			$logo_image = crework_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '') );
		return $logo_image;
	}
}

// Return header video (if set)
if ( !function_exists('crework_get_header_video') ) {
	function crework_get_header_video() {
		$video = '';
		if (apply_filters('crework_header_video_enable', !wp_is_mobile() && is_front_page())) {
			if (crework_check_theme_option('header_video')) {
				$video = crework_get_theme_option('header_video');
...
if ( !function_exists('crework_show_post_meta') ) {
	function crework_show_post_meta($args=array()) {

		$args = array_merge(array(
			'components' => 'date,categories,author,counters,share,edit',
		...
		?><div class="post_meta"><?php
			...
			foreach ($components as $comp) {
				...
				// Post categories
				if ($comp == 'categories') {
					$cats = get_post_type()=='post' ? get_the_category_list(', ') : apply_filters('crework_filter_get_post_categories', '');
					if (!empty($cats)) {
						?>
						<span class="post_meta_item post_categories"><span class="post_categories_in"><?php echo esc_html__('in ', 'crework')?> </span><?php crework_show_layout($cats); ?></span>
						<?php
					}

				// Post date
				} else if ($comp == 'date') {
					$dt = apply_filters('crework_filter_get_post_date', crework_get_date());
					if (!empty($dt)) {
						?>
				...
				// Post author
				 ...
				 // Post counters
                    ...
                    if ( ($output = crework_get_post_counters($args['counters'])) != '')
                        crework_show_layout($output);
                    else {
                        if (!is_singular() || have_comments() || comments_open()) {
                            $post_comments = get_comments_number();
                            ?>
                            <a href="<?php comments_link(); ?>" class="post_meta_item post_counters_item post_counters_comments icon-chat-comment-oval-speech-bubble-with-text-lines"><?php
                                ?><span class="post_counters_number"><?php
                                    echo esc_html($post_comments);
                                ?></span><?php
                                ?><span class="post_counters_label"><?php
                                    echo esc_html(($post_comments === 1) ? esc_html__('Comment', 'crework') :  esc_html__('Comments', 'crework'));
                                ?></span>
                            </a>
                            <?php
                        }
                ...
                // Socials share
					...
					crework_show_share_links(array(
							'type' => 'drop',
							'caption' => esc_html__('Share', 'crework'),
				...
				// Edit page link
					...
					edit_post_link( esc_html__( 'Edit', 'crework' ), '<span class="post_meta_item post_edit">', '</span>' );
				}
			}
		?></div><!-- .post_meta --><?php
...
if ( !function_exists('crework_show_post_featured') ) {
	function crework_show_post_featured($args=array()) {

		$args = array_merge(array(
			'hover' => crework_get_theme_option('image_hover'),	// Hover effect
			...
			'seo' => crework_is_on(crework_get_theme_option('seo_snippets')),
			'singular' => is_singular()						// Current page is singular (true) or blog/shortcode (false)
			), $args);

		if ( post_password_required() ) return;

		$thumb_size = !empty($args['thumb_size']) ? $args['thumb_size'] : crework_get_thumb_size(is_attachment() ? 'full' : (is_single() ? 'huge' : 'big'));
		$post_format = str_replace('post-format-', '', get_post_format());
		$no_image = !empty($args['show_no_image']) ? crework_get_no_image() : '';
		if ($args['thumb_bg']) {
			if (has_post_thumbnail()) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id(get_the_ID()), $thumb_size );
				$image = $image[0];
			} else if ($post_format == 'image') {
				$image = crework_get_post_image();
				if (!empty($image)) 
					$image = crework_add_thumb_size($image, $thumb_size);
			}
			if (empty($image))
				$image = $no_image;
			if (!empty($image))
				$args['class'] .= ($args['class'] ? ' ' : '') . 'post_featured_bg' . ' ' . crework_add_inline_css_class('background-image: url('.esc_url($image).');');
		}
			...
			if ( is_attachment() ) {
				...
				<div class="post_featured<?php if ($args['class']) echo ' '.esc_attr($args['class']); ?>"<?php if ($args['seo']) echo ' itemscope itemprop="image" itemtype="//schema.org/ImageObject"'; ?>>
					...
					if (has_post_thumbnail() && $args['seo']) {
						$crework_attr = crework_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
						?>
			...
			if ($has_thumb 
				...
				if ( $args['thumb_bg'] ) {
					...
					if (!in_array($post_format, array('audio', 'video'))) {
						crework_hovers_add_icons($args['hover']);
					}
					...
					if (($output=crework_get_slider_layout(array('thumb_size'=>$thumb_size, 'controls'=>'yes', 'pagination'=>'yes'))) != '')
						crework_show_layout($output);
						?>
						<div class="post_featured_meta">
							<?php
					...
					if (!empty($image)) {
						...
						crework_hovers_add_icons($args['hover'], array('image' => $image));
					}
				...
				if (!$args['thumb_only'] && ($post_format == 'video' || $post_format == 'audio')) {
					...
					if ($post_format == 'video') {
						$video = crework_get_post_video($post_content, false);
						if (empty($video))
							$video = crework_get_post_iframe($post_content, false);
                        if ( empty( $video ) ) {
                            $post_content = crework_get_post_content();
                            ...
                            if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
                                ...
                                if ( ! empty( $videos ) && is_array( $videos ) ) {
                                    $video = crework_array_get_first( $videos, false );
                                }
                            }
                        }
						if (!empty($video)) {
							if ( $has_thumb ) {
								$video = crework_make_video_autoplay($video);
								?><div class="post_video_hover" data-video="<?php echo esc_attr($video); ?>"></div><?php 
							}
							?><div class="post_video video_frame"><?php 
								if ( !$has_thumb ) {
									crework_show_layout($video);
								}
							?></div><?php
						}
					...
					// Put audio over the thumb
						...
						$audio = crework_get_post_audio($post_content, false);
						if (empty($audio))
							$audio = crework_get_post_iframe($post_content, false);
						if (!empty($audio)) {
							//Show metadata (for the future version)
							if (false && function_exists('wp_read_audio_metadata')) {
								$src = crework_get_post_audio($audio);
							...
							?><div class="post_audio<?php if (strpos($audio, 'soundcloud')!==false) echo ' with_iframe'; ?>"><?php 
								...
								$media_author = crework_get_theme_option('media_author', '', false, get_the_ID());
								$media_title = crework_get_theme_option('media_title', '', false, get_the_ID());
								if ( !empty($media_author) && !crework_is_inherit($media_author) ) {
									?><div class="post_audio_author"><?php crework_show_layout($media_author); ?></div><?php
								}
								if ( !empty($media_title) && !crework_is_inherit($media_title) ) {
									?><h5 class="post_audio_title"><?php crework_show_layout($media_title); ?></h5><?php
								}
								// Display audio
								crework_show_layout($audio); 
							?></div><?php
						}
					}
				}
...
if ( ! function_exists( 'crework_get_post_content' ) ) {
    function crework_get_post_content( $apply_filters=false ) {
        global $post;
...
if ( !function_exists('crework_get_no_image') ) {
	function crework_get_no_image($no_image='') {
		static $img = '';
		if (empty($img)) {
			$img = crework_get_theme_option( 'no_image' );
			if (empty($img)) $img = crework_get_file_url('images/no-image.jpg');
		}
...
if ( !function_exists('crework_add_bg_in_post_nav') ) {
	function crework_add_bg_in_post_nav() {
		if ( ! is_single() ) return;
	
		$previous = ( is_attachment() ) ? get_post( get_post()->post_parent ) : get_adjacent_post( false, '', true );
		$next     = get_adjacent_post( false, '', false );
		$css      = '';
		$noimg    = crework_get_no_image();
		
		if ( is_attachment() && $previous->post_type == 'attachment' ) return;
	
		if ( $previous ) {
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), crework_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		if ( $next ) {
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), crework_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		wp_add_inline_style( 'crework-main', $css );
	}
}

// Show related posts
if ( !function_exists('crework_show_related_posts') ) {
	function crework_show_related_posts($args=array(), $style=1, $title='') {
		$args = array_merge(array(
		...
		$taxonomy = $args['post_type'] == 'post' ? 'category' : crework_get_post_type_taxonomy();
			...
			<section class="related_wrap">
				<h3 class="section_title related_wrap_title"><?php
					if (!empty($title))
						...
						esc_html_e('You May Also Like', 'crework');
				?></h3>
...
if ( !function_exists('crework_show_portfolio_posts') ) {
	function crework_show_portfolio_posts($args=array()) {
		$args = array_merge(array(
			'cat' => 0,
			'parent_cat' => 0,
			'taxonomy' => 'category',
		...
		$blog_style = explode('_', empty($args['blog_style']) ? crework_get_theme_option('blog_style') : $args['blog_style']);
		$style = $blog_style[0];
		...
		if ( !$args['echo'] ) {
			...
			$q_args = crework_query_add_posts_and_cats($q_args, '', $args['post_type'], $args['cat'], $args['taxonomy']);
			if ($args['page'] > 1) {
				$q_args['paged'] = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = crework_get_theme_option('posts_per_page');
			if ((int) $ppp != 0)
...
// AJAX handler for the crework_ajax_get_posts action
if ( !function_exists( 'crework_ajax_get_posts_callback' ) ) {
	add_action('wp_ajax_crework_ajax_get_posts',			'crework_ajax_get_posts_callback');
	add_action('wp_ajax_nopriv_crework_ajax_get_posts',	'crework_ajax_get_posts_callback');
	function crework_ajax_get_posts_callback() {
		if ( !wp_verify_nonce( crework_get_value_gp('nonce'), admin_url('admin-ajax.php') ) )
			wp_die();
	
		$id = !empty($_REQUEST['blog_template']) ? crework_get_value_gpc('blog_template') : 0;
		if ($id > 0) {
			crework_storage_set('blog_archive', true);
			crework_storage_set('blog_mode', 'blog');
			crework_storage_set('options_meta', get_post_meta($id, 'crework_options', true));
		}

		$response = array(
			'error'=>'', 
			'data' => crework_show_portfolio_posts(array(
							'cat' => (int) crework_get_value_gpc('cat'),
							'parent_cat' => (int) crework_get_value_gpc('parent_cat'),
							'page' => (int) crework_get_value_gpc('page'),
							'post_type' => trim(crework_get_value_gpc('post_type')),
							'taxonomy' => trim(crework_get_value_gpc('taxonomy')),
							'blog_style' => trim(crework_get_value_gpc('blog_style')),
							'echo' => false
							)
						)
		);
		...
		if (empty($response['data'])) {
			$response['error'] = esc_html__('Sorry, but nothing matched your search criteria.', 'crework');
		}
...
// Show pagination
if ( !function_exists('crework_show_pagination') ) {
	function crework_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = crework_get_theme_option('blog_pagination');
		if ($pagination == 'pages') {
			the_posts_pagination( array(
				'mid_size'  => 2,
				'prev_text' => esc_html__( '<', 'crework' ),
				'next_text' => esc_html__( '>', 'crework' ),
				'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'crework' ) . ' </span>',
			) );
			...
			if ($page_number < $wp_query->max_num_pages) {
				...
				<div class="nav-links-more<?php if ($pagination == 'infinite') echo ' nav-links-infinite'; ?>">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e('Load more posts', 'crework'); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__('Previous posts', 'crework') : esc_html__('Newest posts', 'crework') ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__('Next posts', 'crework') : esc_html__('Older posts', 'crework'), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
