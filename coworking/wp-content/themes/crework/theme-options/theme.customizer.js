/* global crework_color_schemes, crework_dependencies, Color */
...
( function( api ) {
	...
	for (var i in crework_color_schemes) {
		cssTemplate[i] = wp.template( 'crework-color-scheme-'+i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'crework-fonts' );
	
	// Set initial state of controls
	api.bind('ready', function() {

		// Add 'reset' button
		jQuery('#customize-header-actions #save').before('<input type="button" class="button customize-action-reset" value="'+crework_customizer_vars['msg_reset']+'">');
		jQuery('#customize-header-actions .customize-action-reset').on('click', function(e) {
			if (confirm(crework_customizer_vars['msg_reset_confirm'])) {
		...
		jQuery('#customize-theme-controls [id^="customize-control-load_fonts"]').on('focusout', crework_customizer_update_load_fonts);

		// Click on the actions button
		jQuery('#customize-theme-controls .control-section .customize-control-button input[type="button"]').on('click', crework_customizer_actions);

		// Check dependencies in the each section
		jQuery('#customize-theme-controls .control-section').each(function () {
			crework_customizer_check_dependencies(jQuery(this));
		});
	});
	
	// On change any control - check for dependencies
	api.bind('change', function(obj) {
		crework_customizer_check_dependencies(jQuery('#customize-theme-controls #customize-control-'+obj.id).parents('.control-section'));
		crework_customizer_refresh_preview(obj);
	});

	// Check for dependencies
	function crework_customizer_check_dependencies(cont) {
		cont.find('.customize-control').each(function() {
			...
			if (depend) {
				...
				if ((dep_cnt > 0 && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
					jQuery(this).show().removeClass('crework_options_no_use');
				} else {
					jQuery(this).hide().addClass('crework_options_no_use');
				}
			}
		});
	}
	...
	function crework_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			// Any theme fonts parameter is changed
				...
				for (opt in crework_theme_fonts) {
					for (rule in crework_theme_fonts[opt]) {
						if (opt+'_'+rule == id) {
							// Store new value in the color table
							crework_customizer_update_theme_fonts(opt, rule, val);
							processed = true;
							break;
						}
					}
			...
			// Refresh CSS
			if (processed) crework_customizer_update_css();
		}
	...
	// Actions buttons
	function crework_customizer_actions(e) {
	...
	function crework_customizer_update_theme_fonts(opt, rule, value) {
		crework_theme_fonts[opt][rule] = value;
	}

	// Change theme fonts options if load fonts is changed
	function crework_customizer_update_load_fonts() {
		var opt_list=[], i, tag, sel, opt, name='', family='', val='', new_val = '', sel_idx = 0;
		updateCSS = false;
		for (i=1; i <= crework_customizer_vars['max_load_fonts']; i++) {
	...
	function crework_customizer_update_css() {
		...
		var fonts = crework_customizer_add_theme_fonts(crework_theme_fonts);

		// Make styles and add into css
		css += crework_customizer_prepare_html_value(cssTemplate['theme_fonts']( fonts ));

		// Add colors
		for (var scheme in crework_color_schemes) {
			
			var colors = [];
			
			// Copy all colors!
			for (var i in crework_color_schemes[scheme].colors) {
				colors[i] = crework_color_schemes[scheme].colors[i];
			}
	...
	function crework_customizer_add_theme_colors(colors) {
		...
		colors.bg_color_061 = Color( colors.bg_color ).toCSS( 'rgba', 0.61 );
		...
		colors.text_dark_01 = Color( colors.text_dark ).toCSS( 'rgba', 0.1 );
		colors.text_dark_015 = Color( colors.text_dark ).toCSS( 'rgba', 0.15 );
		colors.text_dark_031 = Color( colors.text_dark ).toCSS( 'rgba', 0.31 );
		colors.text_dark_05 = Color( colors.text_dark ).toCSS( 'rgba', 0.5 );
		colors.text_dark_058 = Color( colors.text_dark ).toCSS( 'rgba', 0.58 );
		colors.text_dark_07 = Color( colors.text_dark ).toCSS( 'rgba', 0.7 );
		colors.text_link_009 = Color( colors.text_link ).toCSS( 'rgba', 0.09 );
		colors.text_link_02 = Color( colors.text_link ).toCSS( 'rgba', 0.2 );
		colors.text_link_06 = Color( colors.text_link ).toCSS( 'rgba', 0.6 );
		colors.text_link_07 = Color( colors.text_link ).toCSS( 'rgba', 0.7 );
		colors.text_link2_009 = Color( colors.text_link2 ).toCSS( 'rgba', 0.09 );
		colors.text_link2_02 = Color( colors.text_link2 ).toCSS( 'rgba', 0.2 );
		colors.text_link3_009 = Color( colors.text_link3 ).toCSS( 'rgba', 0.09 );
		colors.text_link3_061 = Color( colors.text_link3 ).toCSS( 'rgba', 0.61 );
		colors.text_link4_009 = Color( colors.text_link4 ).toCSS( 'rgba', 0.09 );
		colors.text_link5_009 = Color( colors.text_link5 ).toCSS( 'rgba', 0.09 );
		colors.text_link6_009 = Color( colors.text_link6 ).toCSS( 'rgba', 0.09 );
		colors.text_light_05 = Color( colors.text_light ).toCSS( 'rgba', 0.5 );
		colors.extra_link2_085 = Color( colors.extra_link2 ).toCSS( 'rgba', 0.85 );
		colors.extra_link3_05 = Color( colors.extra_link3 ).toCSS( 'rgba', 0.5 );
		colors.inverse_text_017 = Color( colors.inverse_text ).toCSS( 'rgba', 0.17 );
	...
	function crework_customizer_add_theme_fonts(fonts) {
		...
		for (var tag in fonts) {
			...
			rez[tag+'_font-size'] = typeof fonts[tag]['font-size'] != 'undefined' && fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + crework_customizer_prepare_css_value(fonts[tag]['font-size']) + ";"
												: '';
			...
			rez[tag+'_margin-top'] = typeof fonts[tag]['margin-top'] != 'undefined' && fonts[tag]['margin-top'] != '' && fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + crework_customizer_prepare_css_value(fonts[tag]['margin-top']) + ";"
												: '';
			rez[tag+'_margin-bottom'] = typeof fonts[tag]['margin-bottom'] != 'undefined' && fonts[tag]['margin-bottom'] != '' && fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + crework_customizer_prepare_css_value(fonts[tag]['margin-bottom']) + ";"
												: '';
		}
		return rez;
	}
	...
	// Add ed to css value
	function crework_customizer_prepare_css_value(val) {
		if (val != '' && val != 'inherit') {
			var ed = val.substr(-1);
			if ('0'<=ed && ed<='9') val += 'px';
		}
	...
	function crework_customizer_prepare_html_value(val) {
